...
describe('UmbMultiSig', () => {
  ...
  describe('helpers', () => {
    ...
    it('executes rUMB.startSwapNow()', async () => {
      await rUmb.mock.startSwapNow.withArgs().returns();

      await expect(contract.connect(superOwner).submitRUMBStartSwapNowTx(rUmb.address))
        .to.emit(contract, 'LogExecution').withArgs(1)
    })
